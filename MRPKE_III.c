...
int main(){
 ...
 rbc_61_gabidulin code,code_place;
  rbc_61_mat_fq C, V1;
  rbc_61_qre c, g,c1,g_place;
  rbc_61_vec m,m1;
 ...
 rbc_61_field_init();
  ...
  rbc_61_qre_init_modulus(ELL1);
  rbc_61_mat_fq_init(&C, ELL1, ELL1);
  rbc_61_mat_fq_init(&V1, ELL1, ELL1);
  rbc_61_qre_init(&g);
    rbc_61_qre_init(&g_place);

  rbc_61_qre_init(&c);
  rbc_61_qre_init(&c1);
  rbc_61_vec_init(&m, 3);
  rbc_61_vec_init(&m1, 3);
...
double start,end;
  double dif,dif1;
    ...
    for(int i=0; i < NUM_OF_TEST; i++){
        ...
        rbc_61_qre_set_random(&pk_seedexpander, g);

        rbc_61_gabidulin_init(&code, g, 3, ELL1);
        ...
        rbc_61_vec_set_random(&enc_seedexpander, m, 3);
        rbc_61_gabidulin_encode(c, code, m);

        rbc_61_mat_fq_from_qre(C, c);
        ...
        rbc_61_mat_fq_to_qre(c1, Wf);
        rbc_61_gabidulin_decode(m1, code, c1);


        t4 += now_ms()-t0;
       // printf("\nDecryption done, decrypted\n");
       // rbc_61_vec_print(m1,3);
       // printf("\n Message encrypted was \n");
       // rbc_61_vec_print(m,3);
   ...
   rbc_61_mat_fq_clear(V1);
   rbc_61_vec_clear(m);
   rbc_61_vec_clear(m1);
   rbc_61_qre_clear(c);
   rbc_61_qre_clear(g);
   rbc_61_qre_clear(c1);
   rbc_61_qre_clear(g_place);
    for (int i = 0; i < ELL1; i++) {
