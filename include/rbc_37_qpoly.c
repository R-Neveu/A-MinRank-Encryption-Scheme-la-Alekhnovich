...
/** 
 * \fn int8_t rbc_37_qpoly_set(rbc_37_qpoly o, const rbc_37_qpoly p)
 ...
 */
int8_t rbc_37_qpoly_set(rbc_37_qpoly o, const rbc_37_qpoly p) {
  ...
  for(int32_t i = 0 ; i <= p->degree ; ++i) {
    rbc_37_elt_set(o->values[i], p->values[i]);
  }
...
/** 
 * \fn int8_t rbc_37_qpoly_set_mask(rbc_37_qpoly o, const rbc_37_qpoly p1, const rbc_37_qpoly p2, uint32_t mask)
 ...
 */
int8_t rbc_37_qpoly_set_mask(rbc_37_qpoly o, const rbc_37_qpoly p1, const rbc_37_qpoly p2, uint32_t mask) {
  ...
  for(int32_t i = 0 ; i <= o->degree ; ++i) {
    rbc_37_elt_set_mask1(o->values[i], p1->values[i], p2->values[i], mask);
  }
...
/**
 * \fn int8_t rbc_37_qpoly_set_interpolate_vect_and_zero(rbc_37_qpoly o1, rbc_37_qpoly o2, const rbc_37_vec& p1, const rbc_37_vec& p2, int32_t size);
 ...
 *
 * \param[out] o1 rbc_37_qpoly such that \f$ o1(p1_i) = 0 \f$ with \f$ 0 \leq i \leq size - 1 \f$
 * \param[out] o2 rbc_37_qpoly such that \f$ o2(p1_i) = p2_i \f$ with \f$ 0 \leq i \leq size - 1 \f$
 ...
 */
int8_t rbc_37_qpoly_set_interpolate_vect_and_zero(rbc_37_qpoly o1, rbc_37_qpoly o2, const rbc_37_poly p1, const rbc_37_poly p2, int32_t size) {
  if(o1->max_degree < size || o2->max_degree < size - 1) {
    return INVALID_PARAMETERS;
  }

  rbc_37_elt tmp1, tmp2, tmp3;

  rbc_37_qpoly qtmp;
  rbc_37_qpoly_init(&qtmp, size);

  rbc_37_qpoly_set_one(o1);
  rbc_37_qpoly_set_zero(o2);
  rbc_37_qpoly_set_zero(qtmp);

  for(int32_t i = 0 ; i < size ; ++i)  {
    rbc_37_qpoly_evaluate(tmp1, o1, p1->v[i]);
    rbc_37_qpoly_evaluate(tmp2, o2, p1->v[i]);

    rbc_37_elt_inv(tmp3, tmp1);
    rbc_37_elt_add(tmp2, tmp2, p2->v[i]);
    rbc_37_elt_mul(tmp2, tmp2, tmp3);

    rbc_37_qpoly_scalar_mul(qtmp, o1, tmp2);
    rbc_37_qpoly_add(o2, qtmp, o2);

    rbc_37_qpoly_qexp(qtmp, o1);
    rbc_37_qpoly_scalar_mul(o1, o1, tmp1);
    rbc_37_qpoly_add(o1, o1, qtmp);
  }

  rbc_37_qpoly_clear(qtmp);
...
/** 
 * \fn int8_t rbc_37_qpoly_mul(rbc_37_qpoly o, const rbc_37_qpoly p1, const rbc_37_qpoly p2)
 ...
 */
int8_t rbc_37_qpoly_mul(rbc_37_qpoly o, const rbc_37_qpoly p1, const rbc_37_qpoly p2) {
  if(o->max_degree < p1->degree + p2->degree) {
    return INVALID_PARAMETERS;
  }

  rbc_37_qpoly_set_zero(o);
  rbc_37_elt tmp1, tmp2;

  for(int32_t j = 0 ; j <= p2->degree ; ++j) {
    rbc_37_elt_set(tmp1, p2->values[j]);
    rbc_37_elt_mul(tmp2, p1->values[0], tmp1);
    rbc_37_elt_add(o->values[j], o->values[j], tmp2);
    for(int32_t i = 1 ; i <= p1->degree ; ++i) {
      rbc_37_elt_sqr(tmp1, tmp1);
      rbc_37_elt_mul(tmp2, p1->values[i], tmp1);
      rbc_37_elt_add(o->values[(i + j) % RBC_37_FIELD_M], o->values[(i + j) % RBC_37_FIELD_M], tmp2); 
    }
  }

  rbc_37_qpoly_update_degree(o, p1->degree + p2->degree);
...
/** 
 * \fn int8_t rbc_37_qpoly_mul2(rbc_37_qpoly o, const rbc_37_qpoly p1, const rbc_37_qpoly p2, uint32_t p1_degree, uint32_r p2_degree)
 * \brief This function computes the multiplication of two q_polynomials.
 *
 * Multiplication of two q_polynomials is non commutative and is defined using composition namely \f$ p \times q = p \circ q \f$.
 * Execution time of this function depends on p1_degree and p2_degree rather than p1->degree and p2->degree as in rbc_37_qpoly_mul.
 *
 * \param[out] o q_poynomial equal to \f$ p1 \circ p2 \f$
 * \param[in] p1 q_polynomial
 * \param[in] p2 q_polynomial
 ...
 * \return INVALID_PARAMETERS if <b>o.max_degree < p1.degree + p2.degree</b>, SUCCESS otherwise
 */
int8_t rbc_37_qpoly_mul2(rbc_37_qpoly o, const rbc_37_qpoly p1, const rbc_37_qpoly p2, uint32_t p1_degree, uint32_t p2_degree) {
  if(o->max_degree < p1->degree + p2->degree) {
    return INVALID_PARAMETERS;
  }
...
/** 
 * \fn int8_t rbc_37_qpoly_left_div(rbc_37_qpoly q, rbc_37_qpoly r, const rbc_37_qpoly a, const rbc_37_qpoly b)
 ...
 */
int8_t rbc_37_qpoly_left_div(rbc_37_qpoly q, rbc_37_qpoly r, const rbc_37_qpoly a, const rbc_37_qpoly b) {
  if(q->max_degree < a->degree - b->degree || r->max_degree < b->degree - 1 || rbc_37_qpoly_is_zero(b)) {
    return INVALID_PARAMETERS;
  }
  ...
  rbc_37_qpoly_init(&rtmp, a->degree);
  rbc_37_qpoly_init(&s, a->degree - b->degree);
  rbc_37_qpoly_init(&t, a->max_degree);

  rbc_37_qpoly_set_zero(q);
  rbc_37_qpoly_set(rtmp, a);

  rbc_37_elt b_lead_coeff_inv;
  rbc_37_elt r_lead_coeff;
  rbc_37_elt s_lead_coeff;

  rbc_37_elt_set(b_lead_coeff_inv, b->values[b->degree]);
  rbc_37_elt_inv(b_lead_coeff_inv, b_lead_coeff_inv);

  while(rtmp->degree >= b->degree) {

    // Compute monial s
    rbc_37_elt_set(r_lead_coeff, rtmp->values[rtmp->degree]);
    rbc_37_elt_mul(s_lead_coeff, r_lead_coeff, b_lead_coeff_inv);
    rbc_37_elt_nth_root(s_lead_coeff, s_lead_coeff, b->degree);

    rbc_37_qpoly_set_zero(s);
    rbc_37_elt_set(s->values[rtmp->degree - b->degree], s_lead_coeff);
    s->degree = rtmp->degree - b->degree;

    // Update quotient rbc_37_qpoly
    rbc_37_qpoly_add(q, q, s);

    // Update remainder rbc_37_qpoly
    rbc_37_qpoly_mul(t, b, s);
    rbc_37_qpoly_add(rtmp, rtmp, t);

  }

  rbc_37_qpoly_set(r, rtmp);

  rbc_37_qpoly_clear(rtmp);
  rbc_37_qpoly_clear(s);
  rbc_37_qpoly_clear(t);
...
/** 
 * \fn int8_t rbc_37_qpoly_left_div(rbc_37_qpoly q, rbc_37_qpoly r, const rbc_37_qpoly a, const rbc_37_qpoly b, uint32_t capacity, uint32_t k)
 * \brief This function computes the left division of two q_polynomials using euclidean algorithm: \f$ a = b \times q + r \f$.
 *
 * This function is a variant of rbc_37_qpoly_left_div that uses rbc_37_poly_mul2
 *
 * \param[out] q q_polynomial equal to the quotient of the division
 * \param[out] r q_polynomial equal to the remainder of the division
 * \param[in] a q_polynomial to divide
 ...
 * \return INVALID_PARAMETERS if <b>q.max_degree < a.degree - b.degree</b> or <b>r.max_degree < b.degree - 1</b> or <b>b = 0</b>, SUCCESS otherwise
 */
int8_t rbc_37_qpoly_left_div2(rbc_37_qpoly q, rbc_37_qpoly r, const rbc_37_qpoly a, const rbc_37_qpoly b, uint32_t capacity, uint32_t k) {
  if(q->max_degree < a->degree - b->degree || r->max_degree < b->degree - 1 || rbc_37_qpoly_is_zero(b)) {
    return INVALID_PARAMETERS;
  }
  ...
  while(rtmp->degree >= b->degree) {
    ...
    // Update remainder rbc_37_qpoly
    rbc_37_qpoly_mul2(t, b, s, capacity, i);
    rbc_37_qpoly_add(rtmp, rtmp, t);
...
/** 
 * \fn void rbc_37_qpoly_print(const rbc_37_qpoly p)
 * \brief This function prints a q_polynomial.
 *
 * \param p q_polynomial
 */
void rbc_37_qpoly_print(const rbc_37_qpoly p) {
  if(rbc_37_qpoly_is_zero(p) == 1) {
    printf("0");
  } 
  else {
    for(int32_t i = p->degree; i > 0 ; --i) {
      rbc_37_elt_print(p->values[i]);
      printf("x^[%i] + ", i);
    }
    rbc_37_elt_print(p->values[0]);
    printf(" x^[0]");
  }
  printf("\n");
}
