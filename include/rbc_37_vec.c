...
/**
 * \fn void rbc_37_vec_set_random(random_source*, rbc_37_vec v, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_37_vec_set_random(random_source* ctx, rbc_37_vec v, uint32_t size) {
 ...
 * \param[in] size Size of the vector
 */
void rbc_37_vec_set_random_tmp(seedexpander_shake_t* ctx, rbc_37_vec v, uint32_t size) {
  uint32_t bytes = (RBC_37_FIELD_M % 8 == 0) ? RBC_37_FIELD_M / 8 : RBC_37_FIELD_M / 8 + 1;
  uint8_t random[size * bytes];
  uint8_t mask = (1 << RBC_37_FIELD_M % 8) - 1;
...
/**
 * \fn void rbc_37_vec_set_random_from_xof(rbc_37_vec o, uint32_t size, void (*xof)(uint8_t *, size_t, const uint8_t *, size_t), const uint8_t *xof_input, uint32_t xof_size)
...
/**
 * \fn void rbc_37_vec_set_random_fixed_rank(random_source* ctx, rbc_37_vec o, uint32_t size)
...
void rbc_37_vec_set_random_fixed_rank(random_source* ctx, rbc_37_vec o, uint32_t size, int32_t rank) {
  ...
  while(fixed_rank != rank) {
    rbc_37_vec_set_random(ctx, o, size);
    fixed_rank = rbc_37_vec_get_rank(o, size);
  }
}
...
/**
 * \fn void rbc_37_vec_set_random_full_rank(random_source* ctx, rbc_37_vec o, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_37_vec_set_random_full_rank(random_source* ctx, rbc_37_vec o, uint32_t size) {
...
/**
 * \fn void rbc_37_vec_set_random_full_rank_with_one(random_source* ctx, rbc_37_vec o, uint32_t size) {
...
void rbc_37_vec_set_random_full_rank_with_one_tmp(seedexpander_shake_t* ctx, rbc_37_vec o, uint32_t size) {
  ...
  while(rank != rank_max) {
    rbc_37_vec_set_random_tmp(ctx, o, size - 1);
    rbc_37_elt_set_one(o[size - 1]);
    rank = rbc_37_vec_get_rank(o, size);
  }
}
...
/**
 * \fn void rbc_37_vec_set_random_from_support(random_source* o, rbc_37_vec o, uint32_t size, const rbc_37_vec support, uint32_t support_size)
...
/**
 * \fn void rbc_37_vec_set_random_pair_from_support(random_source* o, rbc_37_vec o1, rbc_37_vec o2, uint32_t size, const rbc_37_vec support, uint32_t support_size)
 ...
 * 
 * \param[out] ctx random source
 * \param[out] o1 rbc_37_vec
 * \param[out] o2 rbc_37_vec
  ...
  if (copy_flag) {
    ...
    while(i != support_size) {
      ...
      if(position < size * (256 / size)) {
        if(random1[j + 1] & 0x1) {
          if(rbc_37_elt_is_zero(o1[position % size])) {
            rbc_37_elt_set(o1[position % size], support[i]);
            i++;
          }
        } 
        else {
          if(rbc_37_elt_is_zero(o2[position % size])) {
            rbc_37_elt_set(o2[position % size], support[i]);
            i++;
          }
        }
      }
    ...
    for(i = 0 ; i < size ; ++i) {
      if(rbc_37_elt_is_zero(o1[i])) {
        for(j = 0 ; j < support_size ; ++j) {
          if(random2[k] & 0x1) {
            rbc_37_elt_add(o1[i], support[j], o1[i]);
          }
    ...
    for(i = 0 ; i < size ; ++i) {
      if(rbc_37_elt_is_zero(o2[i])) {
        for(j = 0 ; j < support_size ; ++j) {
          if(random2[k] & 0x1) {
            rbc_37_elt_add(o2[i], support[j], o2[i]);
          }
...
/**
 * \fn uint32_t rbc_37_vec_echelonize(rbc_37_vec v, uint32_t size)
 ...
 * \return Rank of the vector <b>v</b>
 */
uint32_t rbc_37_vec_echelonize(rbc_37_vec v, uint32_t size) {
  uint32_t dimension = rbc_37_vec_gauss(v, size, 1, NULL, 0);
...
/**
 * \fn void rbc_37_vec_from_bytes_without_mask(rbc_37_vec o, uint32_t size, const uint8_t *random)
 * \brief This function sets a vector of finite field elements from random bytes.
 *
 * \param[out] v rbc_37_vec
 * \param[in] size Size of the vector
