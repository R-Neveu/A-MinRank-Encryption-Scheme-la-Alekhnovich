/** 
 * \file rbc_61_lrpc.cpp
 * \brief Implementation of rbc_61_lrpc.h
 */

#include "rbc_61_lrpc.h"
...
/**
 * @fn         unsigned int rbc_61_lrpc_RSR(rbc_61_vspace E, const uint32_t E_expected_dim, const rbc_61_vspace F, const uint32_t F_dim, const rbc_61_vec ec, const uint32_t ec_size)
 *
...
unsigned int rbc_61_lrpc_RSR(rbc_61_vspace E, const uint32_t E_expected_dim, const rbc_61_vspace F, const uint32_t F_dim, const rbc_61_vec ec, const uint32_t ec_size) {
  ...
  rbc_61_vspace S;
  rbc_61_vspace Si[F_dim];
  rbc_61_elt Fi_inv;
  uint32_t E_dim;

  rbc_61_vspace_init(&S, ec_size);
  rbc_61_vspace_set(S, ec, ec_size);
  rbc_61_vec_gauss(S, ec_size, 0, NULL, 0);
  ...
  for(size_t i = 0 ; i < F_dim ; ++i) {
    rbc_61_vspace_init(&(Si[i]), E_expected_dim * F_dim);
    rbc_61_elt_inv(Fi_inv, F[i]);
    rbc_61_vec_scalar_mul(Si[i], S, Fi_inv, E_expected_dim * F_dim);
  }

  //Intersect every Si
  rbc_61_vspace_intersection(E, Si[0], Si[1], E_expected_dim * F_dim, E_expected_dim * F_dim);

  for(size_t i = 2 ; i < F_dim ; i++) {
    rbc_61_vspace_intersection(E, E, Si[i], E_expected_dim * F_dim, E_expected_dim * F_dim);
  }

  E_dim = rbc_61_vec_echelonize(E, E_expected_dim * F_dim);

  rbc_61_vspace_clear(S);
  for(size_t i = 0 ; i < F_dim ; ++i) {
    rbc_61_vspace_clear(Si[i]);
  }

  #ifdef VERBOSE
    printf("\n\n# Rank Support Recover - Begin #");
    printf("\n\nS: "); rbc_61_vec_print(S, E_expected_dim * F_dim);
    for(size_t i = 0 ; i < F_dim ; ++i) {
      printf("\n\nS%i: ", i); rbc_61_vec_print(Si[i], E_expected_dim * F_dim);
    }
    printf("\n\nE: "); rbc_61_vec_print(E, E_dim);
