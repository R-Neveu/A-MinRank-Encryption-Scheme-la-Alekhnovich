/**
 * \file rbc_61_vec.c
 * \brief Implementation of rbc_61_vec.h
 */
...
/**
 * \fn void rbc_61_vec_init(rbc_61_vec* v, uint32_t size)
 * \brief This function initiates a rbc_61_vec element.
 *
 * \param[out] v Pointer to a rbc_61_vec element
 * \param[in] size Size of the vector
 */
void rbc_61_vec_init(rbc_61_vec* v, uint32_t size) {
  *v = calloc(size, sizeof(rbc_61_elt));
...
/**
 * \fn void rbc_61_vec_clear(rbc_61_vec v)
 * \brief This function clears a rbc_61_vec element.
 *
 * \param[out] v rbc_61_vec
 */
...
/**
 * \fn void rbc_61_vec_set_zero(rbc_61_vec v, uint32_t size)
...
/**
 * \fn void rbc_61_vec_set_random(random_source*, rbc_61_vec v, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_61_vec_set_random(random_source* ctx, rbc_61_vec v, uint32_t size) {
  uint32_t bytes = (RBC_61_FIELD_M % 8 == 0) ? RBC_61_FIELD_M / 8 : RBC_61_FIELD_M / 8 + 1;
  uint8_t random[size * bytes];
  uint8_t mask = (1 << RBC_61_FIELD_M % 8) - 1;
...
/**
 * \fn void rbc_61_vec_set_random(seedexpander_shake_t*, rbc_61_vec v, uint32_t size)
 * \brief This function sets a vector of finite field elements with random values using NIST seed expander.
 *
 ...
 * \param[in] size Size of the vector
 */
void rbc_61_vec_set_random_tmp(seedexpander_shake_t* ctx, rbc_61_vec v, uint32_t size) {
  uint32_t bytes = (RBC_61_FIELD_M % 8 == 0) ? RBC_61_FIELD_M / 8 : RBC_61_FIELD_M / 8 + 1;
  uint8_t random[size * bytes];
  uint8_t mask = (1 << RBC_61_FIELD_M % 8) - 1;
...
/**
 * \fn void rbc_61_vec_set_random_from_xof(rbc_61_vec o, uint32_t size, void (*xof)(uint8_t *, size_t, const uint8_t *, size_t), const uint8_t *xof_input, uint32_t xof_size)
...
void rbc_61_vec_set_random_from_xof(rbc_61_vec o,
  ...
  uint8_t random[size * bytes];
  uint8_t mask = (1 << RBC_61_FIELD_M % 8) - 1;
  if ((RBC_61_FIELD_M % 8) == 0) { mask = 0xff; }
...
/**
 * \fn void rbc_61_vec_set_random_fixed_rank(random_source* ctx, rbc_61_vec o, uint32_t size)
...
void rbc_61_vec_set_random_fixed_rank(random_source* ctx, rbc_61_vec o, uint32_t size, int32_t rank) {
  ...
  while(fixed_rank != rank) {
    rbc_61_vec_set_random(ctx, o, size);
    fixed_rank = rbc_61_vec_get_rank(o, size);
  }
}
...
/**
 * \fn void rbc_61_vec_set_random_full_rank(random_source* ctx, rbc_61_vec o, uint32_t size)
 ...
 * \param[in] size Size of the vector
 */
void rbc_61_vec_set_random_full_rank(random_source* ctx, rbc_61_vec o, uint32_t size) {
  int32_t rank_max = RBC_61_FIELD_M < size ? RBC_61_FIELD_M : size;
  int32_t rank = -1;

  while(rank != rank_max) {
    rbc_61_vec_set_random(ctx, o, size);
    rank = rbc_61_vec_get_rank(o, size);
  }
}
...
/**
 * \fn void rbc_61_vec_set_random_full_rank_with_one(random_source* ctx, rbc_61_vec o, uint32_t size) {
...
void rbc_61_vec_set_random_full_rank_with_one(random_source* ctx, rbc_61_vec o, uint32_t size) {
  ...
  while(rank != rank_max) {
    rbc_61_vec_set_random(ctx, o, size - 1);
    rbc_61_elt_set_one(o[size - 1]);
    rank = rbc_61_vec_get_rank(o, size);
  }
}
...
/**
 * \fn void rbc_61_vec_set_random_full_rank_with_one(seedexpander_shake_t* ctx, rbc_61_vec o, uint32_t size) {
...
void rbc_61_vec_set_random_full_rank_with_one_tmp(seedexpander_shake_t* ctx, rbc_61_vec o, uint32_t size) {
  ...
  while(rank != rank_max) {
    rbc_61_vec_set_random_tmp(ctx, o, size - 1);
    rbc_61_elt_set_one(o[size - 1]);
    rank = rbc_61_vec_get_rank(o, size);
  }
}
...
/**
 * \fn void rbc_61_vec_set_random_from_support(random_source* o, rbc_61_vec o, uint32_t size, const rbc_61_vec support, uint32_t support_size)
...
void rbc_61_vec_set_random_from_support(random_source* ctx, rbc_61_vec o, uint32_t size, const rbc_61_vec support, uint32_t support_size, uint8_t copy_flag) {
  ...
  if(copy_flag) {
    ...
    while(i != support_size) {
      ...
      if(position < size * (256 / size) && rbc_61_elt_is_zero(o[position % size])) {
        rbc_61_elt_set(o[position % size], support[i]);
        i++;
      }
    ...
    uint32_t l = 0;
    for(i = 0 ; i < size ; ++i) {
      if(rbc_61_elt_is_zero(o[i])) {

        for(j = 0 ; j < support_size ; ++j) {
          if(random2[k] & 0x1) {
            rbc_61_elt_add(o[i], support[j], o[i]);
          }
    ...
    for(i=0 ; i<size ; i++) {
      for(j=0 ; j<support_size ; j++) {
        if(random[k] & 0x01) {
          rbc_61_elt_add(o[i], o[i], support[j]);
        }
...
/**
 * \fn void rbc_61_vec_set_random_from_support(seedexpander_shake_t* o, rbc_61_vec o, uint32_t size, const rbc_61_vec support, uint32_t support_size)
 * \brief This function sets a vector with random values using the NIST seed expander. The support of the vector returned by this function is included in the one given in input.
 *
 * \param[out] ctx Seed
 * \param[out] o rbc_61_vec
...
/**
 * \fn void rbc_61_vec_set_random_pair_from_support(random_source* o, rbc_61_vec o1, rbc_61_vec o2, uint32_t size, const rbc_61_vec support, uint32_t support_size)
 ...
 * 
 * \param[out] ctx random source
 * \param[out] o1 rbc_61_vec
 * \param[out] o2 rbc_61_vec
  ...
  if (copy_flag) {
    ...
    while(i != support_size) {
      ...
      if(position < size * (256 / size)) {
        if(random1[j + 1] & 0x1) {
          if(rbc_61_elt_is_zero(o1[position % size])) {
            rbc_61_elt_set(o1[position % size], support[i]);
            i++;
          }
        } 
        else {
          if(rbc_61_elt_is_zero(o2[position % size])) {
            rbc_61_elt_set(o2[position % size], support[i]);
            i++;
          }
        }
      }
    ...
    for(i = 0 ; i < size ; ++i) {
      if(rbc_61_elt_is_zero(o1[i])) {
        for(j = 0 ; j < support_size ; ++j) {
          if(random2[k] & 0x1) {
            rbc_61_elt_add(o1[i], support[j], o1[i]);
          }
    ...
    for(i = 0 ; i < size ; ++i) {
      if(rbc_61_elt_is_zero(o2[i])) {
        for(j = 0 ; j < support_size ; ++j) {
          if(random2[k] & 0x1) {
            rbc_61_elt_add(o2[i], support[j], o2[i]);
          }
...
/**
 * \fn uint32_t rbc_61_vec_gauss(rbc_61_vec v, uint32_t size, rbc_61_vec *other_matrices, uint32_t nMatrices)
...
uint32_t rbc_61_vec_gauss(rbc_61_vec v, uint32_t size, uint8_t reduced_flag, rbc_61_vec *other_matrices, uint32_t nMatrices) {
  ...
  for(uint32_t p = 0 ; p < size ; p++) {
    rbc_61_elt acc;
    rbc_61_elt_set(acc, v[p]);
    for(uint32_t i=p+1 ; i<size ; i++) {
      for(uint32_t j=0 ; j<RBC_61_ELT_DATA_SIZE ; j++) {
        acc[j] |= v[i][j];
      }
    }

    int column = rbc_61_elt_get_degree(acc);
    column += (column < 0);

    dimension += (!rbc_61_elt_is_zero(acc));

    //For each line below
    for(uint32_t i=p+1 ; i < size ; i++) {
        mask = rbc_61_elt_get_coefficient(v[p], column);
        rbc_61_elt_set_mask1(tmp, zero, v[i], mask);
        rbc_61_elt_add(v[p], v[p], tmp);

        for(uint32_t k=0 ; k<nMatrices ; k++) {
          rbc_61_elt_set_mask1(tmp, zero, other_matrices[k][i], mask);
          rbc_61_elt_add(other_matrices[k][p], other_matrices[k][p], tmp);
        }
    }
    ...
    if(reduced_flag) {
      for(uint32_t i=0 ; i < p ; i++) {
        mask = rbc_61_elt_get_coefficient(v[i], column);
        rbc_61_elt_set_mask1(tmp, v[p], zero, mask);
        rbc_61_elt_add(v[i], v[i], tmp);

        for(uint32_t k=0 ; k<nMatrices ; k++) {
          rbc_61_elt_set_mask1(tmp, other_matrices[k][p], zero, mask);
          rbc_61_elt_add(other_matrices[k][i], other_matrices[k][i], tmp);
        }
      }
    }
...
/**
 * \fn uint32_t rbc_61_vec_get_rank(const rbc_61_vec v, uint32_t size)
...
uint32_t rbc_61_vec_get_rank(const rbc_61_vec v, uint32_t size) {
  ...
  rbc_61_vec_init(&copy, size);
  rbc_61_vec_set(copy, v, size);
  dimension = rbc_61_vec_gauss(copy, size, 0, NULL, 0);
  rbc_61_vec_clear(copy);
...
/**
 * \fn uint32_t rbc_61_vec_echelonize(rbc_61_vec v, uint32_t size)
 ...
 * \return Rank of the vector <b>v</b>
 */
uint32_t rbc_61_vec_echelonize(rbc_61_vec v, uint32_t size) {
  uint32_t dimension = rbc_61_vec_gauss(v, size, 1, NULL, 0);
...
/**
 * \fn void rbc_61_vec_inner_product(rbc_61_elt o, const rbc_61_vec v1, const rbc_61_vec v2, uint32_t size)
...
void rbc_61_vec_inner_product(rbc_61_elt o, const rbc_61_vec v1, const rbc_61_vec v2, uint32_t size) {
  rbc_61_elt t;
  rbc_61_elt_set_zero(o);
  for(size_t i = 0 ; i < size ; ++i) {
    rbc_61_elt_mul(t, v1[i], v2[i]);
    rbc_61_elt_add(o, o, t);
  }
  rbc_61_elt_set_zero(t);
}
...
/**
 * \fn void rbc_61_vec_scalar_mul(rbc_61_vec o, const rbc_61_vec v, const rbc_61_elt e, uint32_t size)
 * \brief This functions multiplies a vector of finite field elements by a scalar.
 *
 * \param[out] o rbc_61_vec equal to \f$ e \times v \f$
 * \param[in] v rbc_61_vec
...
/**
 * \fn void rbc_61_vec_to_string(uint8_t* str, const rbc_61_vec v, uint32_t size)
...
void rbc_61_vec_to_string(uint8_t* str, const rbc_61_vec v, uint32_t size) {
  uint32_t bytes1 = RBC_61_FIELD_M / 8;
  uint32_t bytes2 = RBC_61_FIELD_M % 8;
  uint32_t index = bytes1 * size;
  uint32_t str_size = ((size * RBC_61_FIELD_M) % 8 == 0) ? (size * RBC_61_FIELD_M) / 8 : (size * RBC_61_FIELD_M) / 8 + 1;
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      uint8_t bit = rbc_61_elt_get_coefficient(v[i], RBC_61_FIELD_M - j);
      *(str + index) |= (bit << k % 8);
      k++;
      if(k % 8 == 0) index++;
    }
...
/**
 * \fn void rbc_61_vec_from_string(rbc_61_vec v, uint32_t size, const uint8_t* str)
...
void rbc_61_vec_from_string(rbc_61_vec v, uint32_t size, const uint8_t* str) {
  ...
  for(size_t i = 0 ; i < size ; i++) {
    for(size_t j = 1 ; j <= bytes2 ; j++) {
      ...
      rbc_61_elt_set_coefficient_vartime(v[i], RBC_61_FIELD_M - j, bit);
      k++;
      if(k % 8 == 0) index++;
    }
  }
...
/**
 * \fn void rbc_61_vec_from_bytes(rbc_61_vec o, uint32_t size, const uint8_t *random)
...
void rbc_61_vec_from_bytes(rbc_61_vec o, uint32_t size, uint8_t *random) {
  ...
  for(size_t i = 0 ; i < size ; ++i) {
    random[(i + 1) * RBC_61_ELT_UINT8_SIZE - 1] &= mask;
    memcpy(o[i], &random[i * RBC_61_ELT_UINT8_SIZE], RBC_61_ELT_UINT8_SIZE);
  }
}
...
/**
 * \fn void rbc_61_vec_from_bytes_without_mask(rbc_61_vec o, uint32_t size, const uint8_t *random)
 * \brief This function sets a vector of finite field elements from random bytes.
 *
 * \param[out] v rbc_61_vec
 * \param[in] size Size of the vector
...
/**
 * \fn void rbc_61_vec_to_bytes( uint8_t *o, const rbc_61_vec v, uint32_t size)
 * \brief This function sets a vector of finite field elements from random bytes.
 *
...
void rbc_61_vec_to_bytes( uint8_t *o, const rbc_61_vec v, uint32_t size) {
  for(size_t i = 0 ; i < size ; ++i) {
    memcpy(&o[i * RBC_61_ELT_UINT8_SIZE], v[i], RBC_61_ELT_UINT8_SIZE);
  }
}
...
/**
 * \fn void rbc_61_vec_print(rbc_61_vec v, uint32_t size)
 * \brief Display an rbc_61_vec element.
 *
 * \param[out] v rbc_61_vec
 * \param[in] size Size of the vector
 */
void rbc_61_vec_print(rbc_61_vec v, uint32_t size) {
  printf("[\n");
  for(size_t i = 0 ; i < size ; ++i) {
    rbc_61_elt_print(v[i]);
    printf("\n");
  }
  printf("]\n");
}
