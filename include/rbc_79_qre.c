...
/**
 * \fn void rbc_79_qre_clear_modulus(void) {
 * \brief This function clears the quotient ring modulus in use.
 */
void rbc_79_qre_clear_modulus(void) {
  rbc_79_init_qre_modulus--;
...
/**
 * \fn rbc_79_qre_set_random_from_xof(rbc_79_qre o, void (*xof)(uint8_t *, size_t, const uint8_t *, size_t), const uint8_t *xof_input, uint32_t xof_size)
 * \brief This function sets a rbc_79_qre with random values using XOF.
 *
 * \param[out] o rbc_79_qre
 ...
 */
void rbc_79_qre_set_random_from_xof(rbc_79_qre o,
                                 void (*xof)(uint8_t *, size_t, const uint8_t *, size_t),
                                 const uint8_t *xof_input,
                                 uint32_t xof_size) {
  rbc_79_poly_set_random_from_xof(o, RBC_79_QRE_DEG, xof, xof_input, xof_size);
}
...
/**
 * \fn void rbc_79_poly_set_monial(rbc_79_qre o, uint32_t degree)
 * \brief This functions sets a polynomial to x^degree.
 *
 * \param[in] p Polynomial
 * \param[in] degree Degree of the monomial
 */
void rbc_79_qre_set_monomial(rbc_79_qre o, uint32_t degree) {
  rbc_79_poly_set_monomial(o, degree);
}
...
/**
 * \fn void rbc_79_qre_rot(rbc_79_qre v, uint32_t r)
 * \brief This functions rotates entries of a vector with coefficients in GF(q^m).
 *
 * \param[out] v rbc_79_qre with entries rotated by position
 * \param[in] position Integer between 1 and (size - 1) such that v is rotated-right r times
 */
void rbc_79_qre_rot(rbc_79_qre o, rbc_79_qre e, uint32_t rot) {
  ...
  if (rot == (uint32_t)(e->degree + 1)) {
    ...
    rbc_79_qre tmp;
    rbc_79_qre_init(&tmp);
    rbc_79_qre_set_monomial(tmp, rot);
    rbc_79_qre_mul(o, e, tmp);
    rbc_79_qre_clear(tmp);
  }
}
